...
// The "Add content" form markup — identical on admin_content.php and
// index.php's "Add Content" modal (see js/content_form.js, which wires
// up both copies identically via these same element ids). Kept as one
// PHP function rather than duplicated HTML so the two can't drift.
function content_add_form_html(): string
{
    return <<<'HTML'
    <form id="contentForm" class="content-form">
      ...
      <div class="form-row" id="fileRow" style="display:none;">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          ...
          <button type="button" id="pdfExtractBtn" class="ghost-btn" style="display:none;">Extract text from PDF</button>
        </div>
        <p class="meta" id="fileHint" style="display:none; color:var(--muted); font-size:.85rem; margin:4px 0 0;">Select up to 10 related photos to add them as one album — or a single multi-page PDF instead, to add each page as a photo.</p>
        <p class="form-error" id="pdfExtractError" role="alert" style="display:none;"></p>
      </div>
